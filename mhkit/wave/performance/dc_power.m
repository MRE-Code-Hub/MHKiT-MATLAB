...
%         Time series of  measured voltages (V) indexed by time
%
%         if structure: 
...
%         Time series of current (A) indexed by time 
%
%         if structure: 
...
if (isa(voltage,'py.pandas.core.frame.DataFrame')~=1)
    if (isstruct(voltage)==1)
        ...
        ME = MException('MATLAB:dc_power','voltage needs to be a Pandas dataframe, use py.mhkit_python_utils.pandas_dataframe.spectra_to_pandas to create one or create a structure');
        throw(ME);
    end
end
