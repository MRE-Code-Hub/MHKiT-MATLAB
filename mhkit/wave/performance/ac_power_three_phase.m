...
% Parameters
% ------------
%     voltage: structure or pandas dataframe
%         Time series of all three measured voltages (V) indexed by time
%
%         if structure: 
...
%
%     current: structure or pandas dataframe
%         Time series of all three measured current (A) indexed by time 
%
%         if structure: 
...
if (isa(voltage,'py.pandas.core.frame.DataFrame')~=1)
    if (isstruct(voltage)==1)
        ...
        ME = MException('MATLAB:ac_power_three_phase','voltage needs to be a Pandas dataframe, use py.mhkit_python_utils.pandas_dataframe.spectra_to_pandas to create one or a structure');
        throw(ME);
    end
end
