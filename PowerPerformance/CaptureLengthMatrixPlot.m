...
if nargin == 8
    ...
    if any([maxTe <= minTe])
        ME=MException('MATLAB:CaptureLengthMatrixPlot','maxTe needs to be greater than minTe');
        throw(ME);
    end
    
    if any([maxHmo <= minHmo])
        ME=MException('MATLAB:CaptureLengthMatrixPlot','maxHmo needs to be greater than minHmo');
        throw(ME);
    end
else
   minHmo=min(HmoBins);
