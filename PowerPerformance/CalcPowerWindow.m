function powerdata=CalcPowerWindow(voltage,current,time,dateTime,parameters,varargin)
...
if nargin < 5 
    ME = MException('MATLAB:CalcPowerWindow','Incorrect number of input arguments, requires at lest 5 arguments, %d arguments passed',nargin);
    throw(ME);
end

if nargin > 6 
    ME = MException('MATLAB:CalcPowerWindow','Incorrect number of input arguments, too many arguments, requires at most 6, %d arguments passed',nargin);
    throw(ME);   
end
...
if any([length(current) ~= time_size])
    disp(current_size)
    disp(time_size)
...
if nargin == 6
    ...
    if any([timeRange(2)<= timeRange(1), length(timeRange) ~= 2])
