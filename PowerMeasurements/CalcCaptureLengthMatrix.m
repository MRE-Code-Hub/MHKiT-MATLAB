...
if nargin < 10 
    ME = MException('MATLAB:CalcCaptureLengthMatrix','Incorrect number of input arguments, requires 10 arguments, %d arguments passed',nargin);
    throw(ME);
end

if nargin > 10 
    ME = MException('MATLAB:CalcCaptureLengthMatrix',['Incorrect number of input arguments, too many arguments, requires at most 10, %d arguments passed',nargin]);
    throw(ME);   
end
...
if any([~isvector(time),~isnumeric(time),length(time)==1])
    ME=MException('MATLAB:CalcCaptureLengthMatrix','time must be a numeric vector with lenght >1');
    throw(ME);
end
...
% check that the power and timePower arrays are of the same size/dimensions
if ~isequal(length(capLength),length(time))
    ME=MException('MATLAB:CalcCaptureLenghtMatrix','capLength and time arrays must have the same length');
    throw(ME);
end
